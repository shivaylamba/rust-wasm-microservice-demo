...
fn get_url() -> String {
    if let Ok(url) = std::env::var("DATABASE_URL") {
        let opts = Opts::from_url(&url).expect("DATABASE_URL is invalid");
        if opts
            .db_name()
            .expect("a database name is required here")
            .is_empty()
        {
            panic!("database name is empty right now");
        }
        url
    } else {
...
//The tokio and mio crates are used for asynchronous networking applications
